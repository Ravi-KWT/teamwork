...
(function() {
  angular.module('mis').controller('TasksCtrl', function($scope, task, $timeout, DTOptionsBuilder, DTColumnDefBuilder, $window, notify, prompt) {
    ...
    $scope.startTimer = function(task_id, user_id, project_id) {
      $scope.task_id = task_id;
      $scope.project_id = project_id;
      $scope.user_id = user_id;
      task.startLogTimer($scope.task_id, $scope.user_id, $scope.project_id).success(function(data) {
        console.log(data);
        return notify({
    ...
    $scope.pauseTimer = function(task_id, user_id, project_id, timer_id) {
      $scope.task_id = task_id;
      $scope.project_id = project_id;
      $scope.user_id = user_id;
      $scope.timer_id = timer_id;
      task.pauseLogTimer($scope.task_id, $scope.user_id, $scope.project_id, $scope.timer_id).success(function(data) {
        console.log(data);
        return notify({
      ...
      if ($scope.edit === false) {
        ...
        return task.savelog($scope.logtime, pId).success(function(data) {
          if (data.success === true) {
            ...
            return console.log(data.errors.billable[0]);
          }
        });
      } else {
        return task.updatelog($scope.logtime).success(function(data) {
          if (data.success === true) {
            ...
            $scope.loading = false;
            return $scope.Logtime.billable.error = data.errors.billable[0];
          }
        });
      }
    };
    $scope.deleteTask = function(id) {
      ...
      return swal({
        ...
        if (result === true) {
          $scope.loading = true;
          task.destroy(pId, id).success(function(data) {
...
(function() {
  angular.module('mis').factory('task', function($http) {
    return {
      ...
      startLogTimer: function(task_id, user_id, project_id) {
        return $http({
          method: 'POST',
          url: '/start-log-timer',
          headers: {
      ...
      pauseLogTimer: function(task_id, user_id, project_id, timer_id) {
        return $http({
          method: 'POST',
          url: '/pause-log-timer',
          headers: {
